...
def test_next_runs_rolls_over_day() -> None:
    now = datetime(2026, 6, 1, 10, 5, tzinfo=timezone.utc)  # just after the 10:00 slot
    r = next_runs(3, now=now)
    assert [(d.day, d.hour) for d in r] == [(1, 16), (2, 6), (2, 10)]
...
def test_next_runs_before_first_slot() -> None:
    ...
    r = next_runs(3, now=now)
    assert [(d.day, d.hour) for d in r] == [(1, 6), (1, 10), (1, 16)]


def test_schedule_hours_match_workflow() -> None:
    # Guards against silent drift from the documented cron.
    assert SCHEDULE_UTC_HOURS == (6, 10, 16)
